...
function AdminFacilityBooking() {
  ...
  const fetchResources = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8082/all-resources`, {
        headers: {
  ...
  const handleAddResource = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token"); // Assuming the token is stored after login
      await axios.post(`http://localhost:8082/add-resource`, newResource, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleDelete = async (resourceId) => {
    ...
    try {
      await axios.delete(
        `http://localhost:8082/delete-resource/${resourceId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const confirmBooking = async (bookingId) => {
    ...
    try {
      await axios.put(
        `http://localhost:8082/confirm-booking/${bookingId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const cancelBooking = async (bookingId) => {
    ...
    try {
      await axios.delete(
        `http://localhost:8082/cancel-booking/${bookingId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
